...
from tools.scan.cli.commands import cmd_signals, cmd_verify
...
def _run_legacy(args: list[str]) -> int:
    ...
    if run_signals:
        from tools.scan.signals.collector import run_collection

        signals = run_collection(deps)
        print(json.dumps([s.to_dict() for s in signals], indent=2))
        return 0

    from tools.scan.signals.provenance.verifier import run_verification
